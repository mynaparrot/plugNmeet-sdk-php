<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: plugnmeet_auth_artifact.proto

namespace Mynaparrot\PlugnmeetProto;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * Generated from protobuf message <code>plugnmeet.ArtifactDetailsRes</code>
 */
class ArtifactDetailsRes extends \Google\Protobuf\Internal\Message
{
    /**
     * Generated from protobuf field <code>bool status = 1;</code>
     */
    protected $status = false;
    /**
     * Generated from protobuf field <code>string msg = 2;</code>
     */
    protected $msg = '';
    /**
     * Generated from protobuf field <code>optional .plugnmeet.ArtifactInfo artifact_info = 3;</code>
     */
    protected $artifact_info = null;
    /**
     * Generated from protobuf field <code>optional .plugnmeet.PastRoomInfo room_info = 4;</code>
     */
    protected $room_info = null;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type bool $status
     *     @type string $msg
     *     @type \Mynaparrot\PlugnmeetProto\ArtifactInfo $artifact_info
     *     @type \Mynaparrot\PlugnmeetProto\PastRoomInfo $room_info
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\PlugnmeetAuthArtifact::initOnce();
        parent::__construct($data);
    }

    /**
     * Generated from protobuf field <code>bool status = 1;</code>
     * @return bool
     */
    public function getStatus()
    {
        return $this->status;
    }

    /**
     * Generated from protobuf field <code>bool status = 1;</code>
     * @param bool $var
     * @return $this
     */
    public function setStatus($var)
    {
        GPBUtil::checkBool($var);
        $this->status = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>string msg = 2;</code>
     * @return string
     */
    public function getMsg()
    {
        return $this->msg;
    }

    /**
     * Generated from protobuf field <code>string msg = 2;</code>
     * @param string $var
     * @return $this
     */
    public function setMsg($var)
    {
        GPBUtil::checkString($var, True);
        $this->msg = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>optional .plugnmeet.ArtifactInfo artifact_info = 3;</code>
     * @return \Mynaparrot\PlugnmeetProto\ArtifactInfo|null
     */
    public function getArtifactInfo()
    {
        return $this->artifact_info;
    }

    public function hasArtifactInfo()
    {
        return isset($this->artifact_info);
    }

    public function clearArtifactInfo()
    {
        unset($this->artifact_info);
    }

    /**
     * Generated from protobuf field <code>optional .plugnmeet.ArtifactInfo artifact_info = 3;</code>
     * @param \Mynaparrot\PlugnmeetProto\ArtifactInfo $var
     * @return $this
     */
    public function setArtifactInfo($var)
    {
        GPBUtil::checkMessage($var, \Mynaparrot\PlugnmeetProto\ArtifactInfo::class);
        $this->artifact_info = $var;

        return $this;
    }

    /**
     * Generated from protobuf field <code>optional .plugnmeet.PastRoomInfo room_info = 4;</code>
     * @return \Mynaparrot\PlugnmeetProto\PastRoomInfo|null
     */
    public function getRoomInfo()
    {
        return $this->room_info;
    }

    public function hasRoomInfo()
    {
        return isset($this->room_info);
    }

    public function clearRoomInfo()
    {
        unset($this->room_info);
    }

    /**
     * Generated from protobuf field <code>optional .plugnmeet.PastRoomInfo room_info = 4;</code>
     * @param \Mynaparrot\PlugnmeetProto\PastRoomInfo $var
     * @return $this
     */
    public function setRoomInfo($var)
    {
        GPBUtil::checkMessage($var, \Mynaparrot\PlugnmeetProto\PastRoomInfo::class);
        $this->room_info = $var;

        return $this;
    }

}

